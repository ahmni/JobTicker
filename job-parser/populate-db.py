...
#                     """)
#    mycursor.execute("ALTER TABLE companies ADD UNIQUE INDEX(name)")
#
...
#                     INDEX(company_name, position, link)""")

    companies = parseCompanyListing(link)

    for company in companies:
        mycursor.execute("INSERT IGNORE INTO companies (name, link) VALUES (%s, %s)",
        ...
        for position in company.positions:
            mycursor.execute("""INSERT IGNORE INTO company_positions
                             (company_name, position, link) VALUES (%s, %s, %s)
